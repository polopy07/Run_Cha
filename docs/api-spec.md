...
- 캐릭터 배치 상태는 응답에서 `isDeployed`, `deployedTerritoryId`로 제공한다. DB 스키마 변경 후에는 `user_characters.deployed_territory_id`를 기준으로 배치 여부를 판단한다.
...
7. 캐릭터 배치 API 담당과 세부 스펙
8. 캐릭터 배치 효과의 공격/방어/버프 적용 범위
