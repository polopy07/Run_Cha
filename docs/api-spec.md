...
1차 구현에서는 별도 `errorCode` 필드를 사용하지 않고 HTTP status와 `message`를 기준으로 에러를 처리한다. 에러 코드 표준화가 필요해지는 경우 기존 응답과 호환되도록 선택 필드로 확장한다.
...
1차 구현에서는 별도 `/running/start` API를 사용하지 않는다. 앱은 러닝 중 GPS 경로를 로컬에서 기록하고, 종료 시 `started_at`, `ended_at`, `path`를 `/running/finish`에 전달해 서버가 러닝 기록, 포인트, 영토 생성, 대표 캐릭터 경험치를 한 번에 처리한다.
...
| ~~neutralAreaSqm~~ | ~~number~~ | ~~대상 점령 영토 밖 새 면적 응답 필드~~ (1차 스코프 제외) |
...
> ~~대상 점령 영토 밖 새 면적을 `neutralAreaSqm`으로 별도 반환하는 응답 정책~~ (1차 스코프 제외)
...
- ~~직접 러닝으로 영토 활동 조건을 충족한 경우에는 `last_active_at` 갱신을 우선 완화 방식으로 사용한다.~~ (1차 스코프 제외)
- ~~러닝 경로와 영토 겹침 비율을 기준으로 `last_active_at`을 갱신하는 활동 조건~~ (1차 스코프 제외)
...
분해로 획득한 스탯 포인트는 `users.stat_points`에 누적 저장하며, 캐릭터 공격/방어/포인트 효율 스탯 강화 비용으로 사용한다.
...
## 7. 1차 스코프 제외 및 확장 검토 항목

1. ~~직접 러닝 기반 `last_active_at` 갱신 조건과 추가 점령률 회복/완화 공식~~ (1차 스코프 제외)
2. ~~겹치는 영토 포인트 수입 패널티 적용 여부와 `territory_overlaps` 캐시 테이블 도입 여부~~ (1차 스코프 제외)
