...
~~직접 러닝으로 영토 활동 조건을 충족한 경우에는 해당 영토의 `last_active_at` 갱신을 우선 완화 방식으로 사용한다. 영토 활동 조건은 러닝 경로와 해당 영토의 겹침 비율 등을 기준으로 후속 구현에서 확정한다.~~ (1차 스코프 제외)
...
- 영토 점령률은 시간이 지나면 반드시 감소한다. ~~직접 러닝으로 영토 활동 조건을 충족하면 `last_active_at` 갱신을 기본 완화 방식으로 사용하고, 영토 활동 조건과 추가 점령률 회복/완화 공식은 별도 구현에서 결정한다.~~ (1차 스코프 제외)
