...
직접 러닝으로 영토 활동 조건을 충족한 경우에는 해당 영토의 `last_active_at` 갱신을 우선 완화 방식으로 사용한다. 후속 자연 감소 완화는 수비형 배치 캐릭터의 방어 스탯을 기준으로 적용하며, 버프형 캐릭터의 포인트 효율 스탯은 시간당 포인트 수익에만 사용해 역할을 분리한다.
...
- 영토 점령률은 시간이 지나면 반드시 감소한다. 직접 러닝으로 영토 활동 조건을 충족하면 `last_active_at` 갱신을 기본 완화 방식으로 사용하고, 추가 점령률 회복/완화 공식은 별도 구현에서 결정한다.
