...
자연 감소 스케줄러는 매일 자정 실행되며 `last_active_at` 기준 비활동 기간에 따라 점령률을 단계적으로 낮춘다. 4일 이상 비활동 영토는 75%, 8일 이상은 50%, 15일 이상은 25%, 22일 이상은 0%로 갱신한다. 0%가 된 영토는 DB 레코드를 보존하되 지도/목록/랭킹/침략 대상 조회에서는 `area_sqm > 0`과 `occupation_rate > 0` 조건으로 제외한다. 자연 감소 업데이트는 `last_active_at`을 변경하지 않는다.
...
- 캐릭터 배치를 자연 감소 스케줄러에 연동하는 경우 배치 데이터와 자연 감소 로직을 함께 갱신한다.
- 영토 점령률은 시간이 지나면 반드시 감소한다. 직접 러닝 충족 시 자연 감소를 완화하는 조건과 기간은 별도 정책으로 확정한다.
