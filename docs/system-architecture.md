...
- GPS 수집은 러닝 화면 포그라운드 실행 중 처리한다.
- 현재 기획 기준으로 `expo-location` 기반 백그라운드 GPS 추적은 사용하지 않는다.
...
2. 지도는 영토 polygon과 함께 사용자 이름, 면적, 배치 캐릭터 요약 정보를 표시
3. 사용자가 점령된 영토를 선택
4. 앱이 영토 상세 화면을 열고 필요 시 GET /territories/:id 호출
5. 상세 화면은 영토 이름, 보유자 이름, 배치 캐릭터 레벨/스탯 등 상세 정보를 표시
6. 현재 사용자의 영토라면 이름 수정과 캐릭터 배치/회수 진입을 제공
```

메뉴의 내 영토 관리 화면은 `GET /territories/me`를 사용해 현재 사용자의 보유 영토를 조회하고, 영토 이름 수정과 캐릭터 배치/회수 흐름으로 연결한다.
...
- 백그라운드 GPS 추적은 현재 범위에서 제외하며, 러닝 화면 포그라운드 GPS 수집을 기준으로 구현한다.
