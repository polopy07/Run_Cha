...
- 수비형 자연 감소 유예일은 `Math.min(Math.floor((defense_lv - 1) / 5), 3)`으로 계산한다.
...
분해는 한 번에 최대 29개까지 가능하며, 분해 후에도 최소 1개의 캐릭터는 보유해야 한다. 분해로 획득한 스탯 포인트는 `users.stat_points`에 누적 저장하고 캐릭터 공격/방어/포인트 효율 스탯 강화 비용으로 사용한다.
...
방어자 영토의 차집합 결과가 `MultiPolygon`이면 1차 스코프 확정 정책으로 가장 큰 조각만 보존하고 나머지는 삭제해 단일 폴리곤 구조를 유지한다. 여러 조각 보존이 필요해지면 후속 확장에서 DB/API/지도 렌더링 구조를 함께 변경한다.
...
~~직접 러닝으로 영토 활동 조건을 충족한 경우 해당 영토의 `last_active_at`을 갱신하는 완화 방식과 러닝 경로/영토 겹침 비율 기반 활동 조건~~ (1차 스코프 제외)

자연 감소 완화는 영토에 배치된 수비형 캐릭터의 방어 스탯을 기준으로 적용한다. 수비형 자연 감소 유예일은 `Math.min(Math.floor((defense_lv - 1) / 5), 3)`으로 계산하며, `defense_lv` 1~5는 추가 유예 없음, 6~10은 +1일, 11~15는 +2일, 16 이상은 +3일을 적용한다. 버프형 캐릭터의 포인트 효율 스탯은 시간당 포인트 수익에만 사용해 역할을 분리한다.
...
- `difference()` 결과가 `MultiPolygon`이면 현재 문서 기준 가장 큰 조각만 보존하는 1차 스코프 확정 정책을 사용한다.
...
- 영토 점령률은 시간이 지나면 반드시 감소한다. ~~직접 러닝으로 `last_active_at`을 갱신하는 완화 방식과 추가 점령률 회복/완화 공식~~ (1차 스코프 제외)
...
- 1차 구현에서는 장거리 러닝 보정을 `distanceMultiplier = Math.min(1.1 ** distanceKm, 3.0)`으로 처리하며, 별도 장시간 연속 러닝 추가 보정 공식은 적용하지 않는다.
