...
전체 Map을 순회하며 2km 이내인 소켓에만 emit. 유저 수가 적은 현재 단계에서는 충분. 대규모 확장 시 geo-hashing 최적화를 검토할 수 있다.
...
| # | 항목 | 현재 제안 | 비고 |
|---|------|-----------|------|
| 1 | 소켓 위치 전송 주기 | 3초 | 모바일 GPS 업데이트와 소켓 emit 기준 |
| 2 | 근처 유저 범위 | 반경 2km | 게임 특성상 러닝 범위 고려 |
| 3 | 대표 미설정 시 표시 | `character: null` | 앱에서 기본 처리 또는 표시 생략 가능 |
| 4 | 이미지 저장 방식 | 앱 번들 키 (`attack_common`) | 현재 이미지 에셋 구조 기준 |
